...
class JournalController < Sinatra::Base
  ...
  # However, there is no need for this since I am using use Rack::Session::Cookie in my config.ru, therefore mounting the session middleware explicitly for my app.
  # The code for the Sinatra classic application level settings would have been:
  ...
  # set :sessions,
  # key: 'journal.session',  # cookie name
  # expire_after: 3600,      # 1 hour
  # secure: production?,     # only over HTTPS in production
  ...
  get '/' do
    JournalLandingPage.new.to_html
  end
